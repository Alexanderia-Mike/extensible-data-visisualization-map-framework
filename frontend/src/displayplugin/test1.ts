...
class test1 implements DisplayPlugin {
    ...
    loadChart(elementId: string, data: StateData | undefined): void {
        ...
        if (xLabels.length > test1.UPPER_LIMIT) {
            xLabels = xLabels.slice(xLabels.length - test1.UPPER_LIMIT);
            cuttedData = dataList.data.slice(dataList.data.length - test1.UPPER_LIMIT);
            console.log("data too large; cut down to " + test1.UPPER_LIMIT);
        } else {
