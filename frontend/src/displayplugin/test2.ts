...
class test2 implements DisplayPlugin {
    ...
    loadChart(elementId: string, data: StateData | undefined): void {
        ...
        if (xLabels.length > test2.UPPER_LIMIT) {
            xLabels = xLabels.slice(xLabels.length - test2.UPPER_LIMIT);
            cuttedData = dataList.data.slice(dataList.data.length - test2.UPPER_LIMIT);
            console.log("data too large; cut down to " + test2.UPPER_LIMIT);
        } else {
